...
void load_A_frozen(struct variable *node_old, struct variable *node_new, int **size_comp_old, int **size_comp_new, int **A, int **A_red, int **list_added, int **list_tot, int *count,int *count_cl, int run){

    int i,j,label_1;
    
    FILE *fp;
    
    if(run==1) system("rm Adj_freezed.txt"); // when running the first time rm output file
   ...
   */
    ...
    if(run == 1){    /* If the first two clusters are merging for the first time into the GC put both in the Adjacency matrix */
        ...
        for (i = 0; i < N; i++) {
           ...
           // node_old[i].Color = node_old[i].LabCluster;
        ...
        /* WRITE THE ADJ MATRIX -----------*/
                fprintf(fp, "%d %d | %d | ",i,node_old[i].LabCluster,A[i][0]);  // Print the new cluster on the Adj matrix i mod_i / deg_i / neighbours
    ...
    if(run == 2){  // If the first two clusters already merged, add to NoN_in only the new cluster which joined
        ...
        (*count_cl)++;  // increment the label of the cluster, each time you enter in if(run=2)
...
void allocate_Ared_and_list(int ***A_red, int ***list_added,int ***list_tot){
    ...
    *list_tot = (int **)malloc(N * sizeof(int *));
    ...
    for (i = 0; i < N; i++) {
        ...
        (*list_tot)[i] = (int *)calloc(3,sizeof(int));
        (*list_tot)[i][1] = 0;    // set all the cluster value to zero
    }
...
void compare_A_and_write(int **A_red, int **A_middle, int **list_added, int **list_tot, int *count){
    ...
    for(i = 0; i < *count; i++){
        ...
        if(A_red[i][0] != A_middle[k][0]){
            ...
            for(j = 1; j <= A_middle[k][0]; j++){ // for all the neighbours
                ...
                if(list_tot[nn][1]!= 0){ // if the neighbours is in one of the previous clusters -- add it
                    ...
                    fprintf(fp,"%d --> %d | %d %d | %d\n",list_tot[k][0],list_tot[nn][0],list_tot[k][1],list_tot[nn][1],diff_module); // print i, j, mod_i, mod_j, type link = 1,0
                    fprintf(fpF,"%d %d %d %d %d\n",list_tot[k][0],list_tot[nn][0],list_tot[k][1],list_tot[nn][1],diff_module);
                }
            }
        }
...
void draw_graph_final(int *count, int **list_added){
    ...
    sprintf(color_link[0],"cadetblue");
    sprintf(color_link[1],"red");
