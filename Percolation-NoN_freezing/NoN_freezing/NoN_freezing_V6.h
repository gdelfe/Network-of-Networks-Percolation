...
void input_parameters(int argc, const char **argv, char ***file_jumps){
    ...
    if(argc == 6){
        ...
        num_jumps = atoi(argv[3]);  // See the headear of the main.c
    }
    
    sprintf(filename,"%s",argv[4]); // file where the Jij names are stored
    fp = fopen(filename,"r");
    printf("file %s\n",filename);
    if(fp == NULL){
    ...
    *file_jumps = (char **)malloc( (3 * num_jumps) * sizeof(char *));
    for (i = 0; i < (3 * num_jumps); i++) {
...
void count_size_of_clusters(struct variable *node, int ***size_comp, int tot_num_cl, int var, double *GC_before, int N){
    ...
    *size_comp = (int **)calloc((tot_num_cl+1),sizeof(int *));
    ...
    if(var == 0)
        *GC_before = (double)(*size_comp)[1][1]/N ; // Size of the GC before the jump
    
    printf("GC\t\t2nd GC\t\t3rd GC\n");
    
    if(tot_num_cl==1) /* if there is only one component */
        printf("%lf\t0\t0\\n\n",(double)(*size_comp)[1][1]/N); /* Print Size GC */
    
    if(tot_num_cl==2) /* if there are only two components */
        printf("%lf\t%lf\t0\n\n",(double)(*size_comp)[1][1]/N,(double)(*size_comp)[2][1]/N); /* Print Size GC, Size 2nd largest component */
    
    
    if(tot_num_cl>=3){ /* if there are at least 3 components */
        /* Print Label GC, Size GC, Label 2nd largest component, Size 2nd largest component, 3rd */
       // printf("%lf\t%lf\t%lf\t\n\n",(double)(*size_comp)[1][1]/N,(double)(*size_comp)[2][1]/N,(double)(*size_comp)[3][1]/N);
    ...
    if(jump >= 2){  // If the first two clusters already merged, add to NoN_in only the new cluster which joined
        ...
        for (i = 0; i < N; i++) {
            ...
            if(node_new[i].LabCluster == size_comp_new[1][0] && node_old[i].LabCluster != size_comp_old[1][0] ){ // If the node is now in the GC but it wasn't before
                ...
                /* LABELING -----------   */
                A_red[*count][0] = A_old[i][0]; // degree i before the jump
                A_middle_tot[*count][0] = A_middle[i][0];  // degree i at the middle
...
void draw_graph(int **A, struct variable *node, int **list_tot, int var){
...
void compare_A_and_write(int **A_red, int **A_middle_tot, int **list_added, int **list_tot, int *count){
    ...
    int av_degree[3];
    av_degree[0] = av_degree[1] = av_degree[2] = 0;
    ...
    fpNodesGephi = fopen("Gephi_Nodes_list.txt","w");  // file to write the list of nodes to pass to Gephi
    fpEdgesGephi = fopen("Gephi_Edges_list.txt","w");  // file to write the list of edges to pass to Gephi
  ...
  */
    ...
    fprintf(fpNodesGephi,"Id Node Attribute\n"); // Header of Node List to pass to Gephi
    fprintf(fpEdgesGephi,"Source Target Type Attribute\n");  // Header of Edge List to pass to Gephi
    
    fprintf(fp,"# i j | mod_i mod_j | TL \n");
    for(i = 0; i < *count; i++){
        
        fprintf(fpNodesGephi,"%d %d %d\n",list_added[i][0],list_added[i][0],list_added[i][1]);
        ...
        if(A_red[i][0] == A_middle_tot[i][0]){ // If the degree is the same -- just write the neighbours
         //   fprintf(fp,"deg same = %d %d    i = %d\n",A_red[i][0],A_middle_tot[i][0],list_added[i][0]);
            ...
            for(j = 1; j <= A_red[i][0]; j++){ // for all the neighbours
                ...
                fprintf(fpEdgesGephi,"%d %d Undirected %d\n",list_added[i][0],A_red[i][j],0);
                av_degree[0] += 1;
                av_degree[2] += 1;
            }
          ...
          //  fprintf(fp,"deg diff = %d %d    i = %d\n",A_red[i][0],A_middle_tot[i][0],list_added[i][0]);
            ...
            for(j = 1; j <= A_middle_tot[i][0]; j++){ // for all the neighbours
                ...
                if(list_tot[nn][1]!= 0){ // if the neighbours is in one of the previous clusters -- add it
                    ...
                    if(diff_module == 0){
                        fprintf(fp,"%d --> %d | %d %d | %d\n",list_added[i][0],nn,list_added[i][1],list_tot[nn][1],diff_module); // print i, j, mod_i, mod_j, type link = 1,0
                        fprintf(fpF,"%d %d %d %d %d\n",list_added[i][0],nn,list_added[i][1],list_tot[nn][1],diff_module);
                        fprintf(fpEdgesGephi,"%d %d Undirected %d\n",list_added[i][0],nn,diff_module);
                        av_degree[diff_module] += 1;
                        av_degree[2] += 1;
                    }
                    ...
                    if(diff_module == 1){
                        ...
                        fprintf(fpEdgesGephi,"%d %d Undirected %d\n",list_added[i][0],list_tot[nn][0],diff_module);
                        ...
                        fprintf(fp,"%d --> %d | %d %d | %d\n",nn,list_added[i][0],list_tot[nn][1],list_added[i][1],diff_module); // print i, j, mod_i, mod_j, type link = 1,0
                        fprintf(fpF,"%d %d %d %d %d\n",nn,list_added[i][0],list_tot[nn][1],list_added[i][1],diff_module);
                        fprintf(fpEdgesGephi,"%d %d Undirected %d\n",nn,list_added[i][0],diff_module);
    ...
    printf(" Num tot nodes = %d\n num tot link = %d\n num tot inlink = %d\n rate inlink = %lf\n num tot outlink = %d\n rate outlink = %lf\n",*count,av_degree[2]/2,av_degree[0]/2, (double)av_degree[0]/(*count)/2,av_degree[1]/2,(double)av_degree[1]/(*count)/2);
...
void draw_graph_final(int *count, int **list_added){
    ...
    char color_node[8][100]; // color array for the nodes
    char color_link[2][100]; // color array for the two types of links
    ...
    system(filename);
   // sprintf(filename,"rm graph_final.dat");
   // system(filename);
...
void print_nodes(struct variable *node_old, struct variable *node_new, int **size_comp_new, int **list_added,int *count,const char **argv){
    ...
    fp = fopen(filename,"w");
        ...
        for (i = 0; i < N; i++) {
            ...
            if((node_new[i].LabCluster == size_comp_new[1][0]) && (node_old[i].SizeCluster > 3)){ // if the label of the new node is equal to the label of GC and the cluster size is larger than a certan value
                
                printf("node = %d, label = %d, size = %lf \n",i,node_old[i].LabCluster,(double)node_old[i].SizeCluster/N);
              ...
              //  label_1 = node_old[i].LabCluster;
                ...
                list_added[*count][0] = i;   /* add node i to the added_node_list consecutively */
                list_added[*count][1] = node_old[i].LabCluster;
                
                //   fprintf(fp,"%d %d\n",i,node_old[i].LabCluster); // print node and node cluster
            /*    if(node_old[i].LabCluster == label_1){
                    node_old[i].LabCluster = 1;  // label i with 1, i.e. first cluster
                    list_added[*count][1] = 1; // same as above in the addition order
                }
    ...
    for(i=0; i< *count; i++){
        fprintf(fp,"%d %d\n",list_added[i][0],list_added[i][1]); // print node and node cluster
    }
