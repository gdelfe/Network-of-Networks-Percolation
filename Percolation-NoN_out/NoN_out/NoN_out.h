...
/*                          Read matrix A_in.                                  */
 ...
 /* Structure of input A_in = cluster_i / degree_i / i / j_1 / j_2 / etc ....  */

int read_allocate_Aij_in(const char **argv, int ***A_in){
    ...
    sprintf(filename,"%s",argv[3]); // read name_file from stdin, file 1 or 2 depending on var
    fp=fopen(filename,"r");
    
    if(fp==NULL){
        fprintf(stderr,"PROBLEM OPENING INPUT FILE A_in \n");
        exit(errno);
    }
    
    
    /* Count tot numb of lines in A_in, i.e. tot number of rows for A_in to be allocated */
    tot_lines = 0;
    ...
    *A_in = (int **)malloc( tot_lines * sizeof(int *)); // Allocate M lines for A_in
    for(i=0;i<tot_lines;i++)
        (*A_in)[i] = (int *)malloc(tot_lines * sizeof(int)); // Allocate only one column per row
    
    
    
    /* Read file again and load A_in */
    fp=fopen(filename,"r");
    ...
    while(fgets(line,MAX_LENGTH,fp) != NULL){
            start = line;
            j=0;
        while(sscanf(start,"%d%n",&(*A_in)[i][j],&n) == 1){
                j++;
               // (*A_in)[i] = (int *) realloc((*A_in)[i], j * sizeof(int)); // Reallocate space for A equal to the tot numb of column, i.e. cluster_i / deg_i / i / j_1 / j_2 / etc...
...
/* =========================================================================== */
/*                          Read matrix J_tot and create A_tot                  */
/* =========================================================================== */
...
void read_allocate_Jij_Aij(const char **argv, double ***J, int ***A){
    ...
    sprintf(filename,"%s",argv[4]); // read name_file from stdin, file 1 or 2 depending on var
    fp=fopen(filename,"r");
    ...
    *A = (int **)malloc( N * sizeof(int *));
    ...
    for( i = 0; i < N; i++){
        ...
        (*A)[i] = (int *)malloc( N * sizeof(int)); // Allocate space for: cluster_i, deg_i, i
        (*A)[i][0] = 0.; // set the cluster of node i to 0
        (*A)[i][1] = 0.; // set the degree of node i to 0
    ...
    for (i = 0; i < N; i++){
        ...
        non_zeros = 3;  // for cluster_i, degree_i, i
        for (j=0; j < N; j++){ // read one line
         fscanf(fp, "%lf",& (*J)[i][j]); // read element
           ...
           //     (*A)[i] = (int *) realloc((*A)[i], (non_zeros+1) * sizeof(int)); // Reallocate space for A equal to the tot numb of non zero element + 1 for the degree
                (*A)[i][1] = (*A)[i][1] + 1;     // increment the degree of the node i, stored in A[i][1]
                (*A)[i][non_zeros] = j;     // Assign j to Aij
...
/*******************************************************************************/
 ...
 /* Structure of input A = cluster_i / degree_i / i / j_1 / j_2 / etc ....  */

void compare_A_in_A_tot(int **A_in, int lines, int **A_tot){

    int i,j,nn, diff_module;
    
    FILE *fp;
    
    fp=fopen("Adj_tot.txt","w");
    ...
    for(i=0; i < N; i++){
        ...
        if(A_in[i][1] == A_tot[i][1]){ // If the degree is the same
            printf("deg same = %d %d    i = %d\n",A_in[i][1],A_tot[i][1],A_in[i][2]);
         //   printf("deg = %d %d i = %d\n",A_in[i][1],A_tot[i][1],A_in[i][2]);
            for(j = 3; j < (A_in[i][1] + 3); j++) // for all the neighbours
                fprintf(fp,"%d --> %d | %d %d | %d\n",A_in[i][2],A_in[i][j],A_in[i][0],A_in[i][0],0); // print i, j, mod_i, mod_j, type link = 1
        }
       
        if(A_in[i][1] != A_tot[i][1]){ // if the degree is different
            printf("deg diff = %d %d    i = %d\n",A_in[i][1],A_tot[i][1],A_in[i][2]);
            
            for(j = 3; j < (A_tot[i][1] + 3); j++){ // for all the neighbours
                nn = A_tot[i][j];
                if(A_in[nn][2] != -1){ // if the neighbours is in one of the previous clusters -- add it
                    
                    diff_module = (A_in[i][0])!=(A_in[nn][0]); // check if the two nodes are in the same cluster. Result: 1 if different, 0 if the same
                    
                    fprintf(fp,"%d --> %d | %d %d | %d\n",A_tot[i][2],A_tot[i][j],A_in[i][0],A_in[nn][0],diff_module); // print i, j, mod_i, mod_j, type link = 1
                }
            }
        }
...
void delta_size(struct variable *node_old, struct variable *node_new, int **size_comp_old, int **size_comp_new, int **A,int run){
...
//    char filename[101];
  //  sprintf(filename, "cluster",var);
    ...
    /* If the size of the node is equal to Size GC, then it has been merged with GC */
    /* Add to the NoN_in file, the A matrix of the nodes which joined GC, as it was before the merging with the GC */

    if(run == 1){    /* If the first two clusters are merging for the first time into the GC */
        for (i=0; i<N; i++) {
            
            if(node_new[i].LabCluster == size_comp_new[1][0]){ // if the label of the new node is equal to the label of GC
                
                if (i!= 0 && node_old[i].LabCluster != node_old[i-1].LabCluster) fprintf(fpA,"-----------------\n");
                
                fprintf(fpA, "%d %d ",node_old[i].LabCluster,i);  // Print to file the Adjacency matrix of the nodes which joined the GC: cluser_i / i / j_1, j_2, etc...
                for (j=1; j <= A[i][0]; j++) {
                    fprintf(fpA,"%d ",A[i][j]);
                }
                fprintf(fpA,"\n");
    ...
    count = 0;
    if(run == 2){  /* If the first two clusters already merged, add to NoN_in only the new clusters which joined */

        for (i=0; i<N; i++) {
            if(node_new[i].LabCluster == size_comp_new[1][0] && node_old[i].LabCluster != size_comp_old[1][0] ){ // If the node is in the GC but it wasn't before
                ...
                fprintf(fpA, "%d %d ",node_old[i].LabCluster,i);            // Print to file the Adjacency matrix of the nodes which joined the GC
                for (j=1; j <= A[i][0]; j++) {
                    fprintf(fpA,"%d ",A[i][j]);
                }
                fprintf(fpA,"\n");
...
void free_memory(double **J_old,double **J_new,int **A_old,int **A_new,int *queue,int **size_comp_old,int **size_comp_new,struct variable *node_old,struct variable *node_new){
