...
void input_parameters(int argc, const char **argv){
    ...
    if(argc == 6){
        ...
        run=atoi(argv[5]);  // See the headear of the main.c
    }
...
//    char filename[101];
  //  sprintf(filename, "cluster",var);
    ...
    if(run==1) system("rm Adj.txt Adj_reduced.txt");
    
    fp = fopen("Adj.txt","a");
    fp_red = fopen("Adj_reduced.txt","a");
    
    if(fp == NULL || fp_red == NULL){
        ...
        for (i=0; i<N; i++) {
            ...
            if(node_new[i].LabCluster == size_comp_new[1][0]){ // if the label of the new node is equal to the label of GC
             ...
             //   if (i!= 0 && node_old[i].LabCluster != node_old[i-1].LabCluster) fprintf(fpA,"-----------------\n");
                ...
                fprintf(fp, "%d %d %d ",node_old[i].LabCluster,A[i][0],i);  // Print to file the Adjacency matrix of the nodes which joined the GC: cluser_i / i / j_1, j_2, etc...
                fprintf(fp_red, "%d %d %d ",node_old[i].LabCluster,A[i][0],i);
                for (j=1; j <= A[i][0]; j++) {
                    fprintf(fp,"%d ",A[i][j]);
                    fprintf(fp_red,"%d ",A[i][j]);
                }
                fprintf(fp,"\n");
                fprintf(fp_red,"\n");
            }
            else fprintf(fp,"-1 -1 -1\n");
        }
    }
